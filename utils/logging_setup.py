...
def _cleanup_old_logs(log_dir: Path, logger: logging.Logger) -> None:
    ...
    """
    try:
        log_files: List[Path] = list(log_dir.glob('py_i18n_*.log'))
        if len(log_files) <= 10:
            return
        ...
        for log_file in log_files:
            try:
                # Extract date from filename (format: py_i18n_YYYY-MM-DD.log)
...
def get_logger(module_name: str) -> logging.Logger:
    ...
    logger: logging.Logger = logging.getLogger(f"py_i18n_manager.{module_name}")
    ...
    log_dir: Path = Path(appdata_dir) / 'py_i18n_manager' / 'logs'
    ...
    log_file: Path = log_dir / f'py_i18n_{date_str}.log'
